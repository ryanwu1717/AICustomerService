  ...
  function addAIReply(){
    console.log("addAIReply")

    $('[name="chatBox"]').append(`
          <div class="d-flex bd-highlight" >
            <div style="max-width:75%" class="p-2 bd-highlight bg-dark  rounded text-break;"  name = "aiReplyChatbox" >
              <ul class="list-group">
                <li class="list-group-item active" aria-current="true">健康</li>
 ...
 starNotificationOnclick =  function (chatID, chatName, id, attr) {
...
starOnclick = function (cliclTime, chatID, content, button) {
  ...
  function changeChatroom(type, data) {
    ...
    if (type == 'init') {
      ...
      console.log(data)
      console.log(data.result.chatroom);
...
onclickHeart =  function(button, senttime) {
 ...
 getTarget = function (_chatID, _chatName) {
...
notificationOnclick = function (chatID, chatName, id, attr, typeNotice) {
...
addDepartmentTag =   function(tagname, tagID) {
    var textAreaContent = $("#textinput").val();
    for (i = $("#textinput").getCursorPosition(); i > 0; i--) {
      if ($("#textinput").val().charAt(i - 1) == "#" || $("#textinput").val().charAt(i - 1) == "＃") {
        $("textarea#textinput").val($("#textinput").val().substr(0, i - 1) + "#" + tagname + " " + $("#textinput").val().substr(i));
...
sendComment =  function (commentID, senttime) {
    if ($("#commentinput").val() != "") {
      Msg = $("#commentinput").val();
      Msg = Msg.replace(/\r?\n/g, '<br />');
      $.ajax({
